...
public final class CarvePumpkinAttraction extends Attraction<CarvePumpkinAttraction.SaveTag> {
    protected Duration gameTime = Duration.ofSeconds(90);
    ...
    public void onEnable() {
        ...
        if (raw.get("maxBlocks") instanceof Number number) {
            this.maxBlocks = number.intValue();
        }
    }
    ...
    private void placeBlocks() {
        ...
        final int max = Math.min(blockList.size(), maxBlocks);
        for (int i = 0; i < max; i += 1) {
            Vec3i vector = blockList.get(i);
            saveTag.blockList.add(vector);
            saveTag.blockDataList.add(vector.toBlock(world).getBlockData().getAsString(false));
    ...
    private void clearBlocks() {
        for (int i = 0; i < saveTag.blockList.size(); i += 1) {
            final Vec3i vector = saveTag.blockList.get(i);
            final BlockData blockData = Bukkit.createBlockData(saveTag.blockDataList.get(i));
            vector.toBlock(world).setBlockData(blockData, false);
        }
        saveTag.blockList.clear();
        saveTag.blockDataList.clear();
    ...
    protected State tickGame() {
        ...
        if (searchTime > gameTime.toMillis()) {
            timeout(player);
            return State.IDLE;
        }
        secondsLeft = (int) ((gameTime.toMillis() - searchTime - 1) / 1000L + 1L);
        updateBlocks();
        ticks += 1;
        return null;
    }
    ...
    protected static final class SaveTag extends Attraction.SaveTag {
        ...
        protected List<String> blockDataList = new ArrayList<>();
